...
public class FakeMessageTransmitter : ITransmitter
{
    ...
    {
        await Initialize();
        await _ghostManagerPublisher.BringOnlineAsync(ghostId);
    }

    public async Task TakeOffline(int ghostId)
    {
        await _ghostManagerPublisher.TakeOfflineAsync(ghostId);
    }
    ...
    public async Task TransmitLocation(int ghostId)
    {
        ...
        await _pathFinderPublisher.TransmitLocationAsync(ghostId, newLocation);
    }
